...
from tensorflow.keras.layers import RNN, LSTMCell, Masking
...
class Actor_Critic:
    ...
    def __init__ (self, Environment, Actor_Network_Size, Actor_Learning_Rate, Actor_Batch, Actor_Activation, Actor_Alpha, Actor_Epoch,
                  Critic_Network_Size, Critic_Learning_Rate, Critic_Batch, Critic_Activation, Critic_Alpha, Critic_Epoch, Gamma, Sigma_Range, 
                  Sigma_Anneal, Retrain_Frequency, Action_Space_Clip = 10, Experiance_Mode = 'Monte_Carlo', TD_Lambda = [1, 0.8, 0.5],
                  Monte_Carlo_Frac = 0.1, Ignore_Actor_Frac = 0.0, thread = 0):
        ...
        self.Actor_Network_Size     = Actor_Network_Size
        self.Actor_Learning_Rate    = Actor_Learning_Rate
        self.Actor_Batch            = Actor_Batch
        self.Actor_Activation       = Actor_Activation
        self.Actor_Alpha            = Actor_Alpha
        self.Actor_Epoch            = Actor_Epoch
        
        self.Critic_Network_Size    = Critic_Network_Size
        ...
        self._OOS_Buffer = np.zeros([1, self.State_Dim])

        with tf.variable_scope(str(thread), reuse=tf.AUTO_REUSE):
            self.Actor  = Policy_Network(self.Actor_Network_Size, self.Episode_Length, self.State_Dim, self.Action_Dim,
                                         Alpha = self.Actor_Alpha, Activation= self.Actor_Activation)
            
            self.Critic = Value_Network(self.Critic_Network_Size, self.Episode_Length, self.State_Dim, 1,
                                        Activation    = self.Critic_Activation,
                                        Alpha         = self.Critic_Alpha)
    ...
    def Train (self, N_Episodes, Plot = Empty, Diag = Empty):
        ...
        for i in tqdm.tqdm(range(N_Episodes)):
            Done = False
            Episode_States = np.zeros([self.Episode_Length, self.State_Dim])
            State_0 = self.Environment.reset()
            step = 0
            Episode_States[step, :] = State_0
            Sigma = max(self.Sigma_Range[0] - ((self.Sigma_Range[0] - self.Sigma_Range[1]) * (i / (self.Sigma_Anneal * N_Episodes))), self.Sigma_Range[1])
            Episode_Exp = []
            while Done == False:
                Mu = self.Predict_Action(Episode_States).flatten()
                Leverage = np.random.normal(Mu, Sigma)
                State_1, Reward, Done, Info = self.Environment.step(Leverage)
                New = {"s0" : deepcopy(Episode_States), "r" : Reward, "a" : Leverage, 'i' : Info, 'Mu' : Mu, 'Sigma' : Sigma, 'd' : Done}
                step += 1
                if not Done: Episode_States[step, :] = State_1
                New['s1'] = deepcopy(Episode_States)
                Step_Count += 1
                Episode_Exp.append(New)
    ...
    def Fit_Actor (self, State, Action, Advantage, Sigma, LR_Mult):
        for _ in range(self.Actor_Epoch):
            ...
            if self.Actor_Batch >= State.shape[0] or self.Actor_Batch == 0:
                self.TF_Session.run(self.Actor.Fit, feed_dict = {self.Actor.State          : State,
                                                                  ...
                                                                  self.Actor.Learning_Rate : self.Actor_Learning_Rate * LR_Mult})

            else:
                idx = np.random.choice(State.shape[0], size = (State.shape[0] // self.Actor_Batch, self.Actor_Batch), replace = False)    
    ...
    def Fit_Critic (self, State, Value):
        for _ in range(self.Critic_Epoch):
            ...
            if self.Critic_Batch >= State.shape[0] or self.Critic_Batch == 0:
                self.TF_Session.run(self.Critic.Fit, feed_dict = {self.Critic.State         : State,
                                                                  self.Critic.Value         : Value,
                                                                  self.Critic.Learning_Rate : self.Critic_Learning_Rate})

            else:
                idx = np.random.choice(State.shape[0], size = (State.shape[0] // self.Critic_Batch, self.Critic_Batch), replace = False)
                for i in range(idx.shape[0]):
    ...
    def Gen_Experiance (self, Episode_Exps, Episode_Frac):
        ...
        if self.Experiance_Mode == 'Monte_Carlo' or Episode_Frac < self.Monte_Carlo_Frac:
            ...
            State     = np.array([e['s0'] for e in Exp]).reshape((-1, self.Episode_Length, self.State_Dim))
            ...
            State_0   = np.array([e['s0'] for e in Exp]).reshape((-1, self.Episode_Length, self.State_Dim))
    ...
    def Predict_Action (self, X, OOS = False):
        ...
        '''
        if OOS:
            if self._OOS_Buffer.shape[0] == 1: 
                self._OOS_Buffer[0] = X
            else:
                self._OOS_Buffer = np.append(self._OOS_Buffer, X)
            if self._OOS_Buffer.shape[0]<self.Episode_Length:
                State_in = np.append(self._OOS_Buffer, np.zeros((self.Episode_Length - self._OOS_Buffer.shape[0], self.State_Dim)), axis = 0)
            else:
                State_in = self._OOS_Buffer[-self.Episode_Length:, :]
            State_in = State_in.reshape(1, self.Episode_Length, self.State_Dim)
        else:
            State_in = X.reshape(-1, self.Episode_Length, self.State_Dim)
        Mu = self.TF_Session.run(self.Actor.Action_Pred, feed_dict = {self.Actor.State : State_in})
    ...
    def Predict_Value (self, X):
        ...
        '''
        X = X.reshape(-1, self.Episode_Length, self.State_Dim)
